...
std::string IniIO::get_value(std::string_view section_name, std::string_view key) const
{
...
std::string IniIO::get_value(std::string_view key) const
{
    std::string value;
...
std::vector<std::pair<std::string, std::string>> IniIO::get_kvps(std::string_view section_name) const
{
...
void IniIO::set_value(std::string_view section_name, std::string_view key, std::string_view value)
{
    auto[s_pos, k_pos] = get_positions(section_name, key);
...
IniIO::get_positions(std::string_view section_name, std::string_view key) const
{
...
ptrdiff_t IniIO::get_section_pos(std::string_view section_name) const
{
    auto s_pos = std::find_if(
            sections.begin(),
            sections.end(),
            [&section_name](const SectionUnit &u) {
                return u.section_name == section_name;
            }) - sections.begin();
...
ptrdiff_t IniIO::get_key_pos(std::string_view key, ptrdiff_t section_pos) const
{
